#!/bin/bash
# =================================================================
# Fix API JSON Response Issue - Restart API with Proper Configuration
# =================================================================

echo "🔧 FIXING API JSON RESPONSE ISSUE"
echo "================================="

echo "🔍 The issue: API is returning HTML instead of JSON"
echo "🎯 Solution: Check API logs and restart with debug info"
echo ""

# First, let's check what's happening in the API logs
echo "📋 Checking current API status..."
curl -s http://localhost:9001/health 2>&1 | head -5

echo ""
echo "🔧 Let's restart the API with better error handling..."

cd ~/pmerit-ai-platform/api

# Create an improved version of the API with better error handling
cat > main_fixed.py << 'FIXED_API_EOF'
# =================================================================
...
echo "✅ Created improved API with better error handling"

# Kill any existing API process on port 9001
echo "🔄 Stopping existing API process..."
pkill -f "uvicorn.*9001" 2>/dev/null || echo "No existing process found"

# Wait a moment
sleep 2

echo "🚀 Starting improved API..."
echo "🔧 Better error handling and JSON responses"
echo "📋 Debug logging enabled"
echo ""

# Start the fixed API
python3 -m uvicorn main_fixed:app --host 0.0.0.0 --port 9001 --reload --log-level info &

# Wait for startup
sleep 3

echo "🧪 Testing improved API..."
...
curl -s http://localhost:9001/api/test | python3 -m json.tool
...
echo "$LOGIN_RESPONSE" | python3 -m json.tool 2>/dev/null || echo "Raw response: $LOGIN_RESPONSE"

# Check if we got a proper JSON response with token
if echo "$LOGIN_RESPONSE" | grep -q "access_token"; then
    echo "🎉 SUCCESS! Login is now working with proper JSON response!"
...
echo "• Test: http://localhost:9001/api/test"
echo "• Login: POST http://localhost:9001/api/auth/login"
