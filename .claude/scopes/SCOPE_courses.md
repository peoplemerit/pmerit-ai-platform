...
SEPARATE from discovery page (SCOPE_Learning_Pathways.md).

### REQUIREMENTS

| ID | Requirement |
|----|-------------|
| R1 | Remove "Career Tracks" section from courses.html |
| R2 | Add pathway filter: `/courses?pathway=data-analytics` |
| R3 | Create course-detail.html with syllabus view |
| R4 | Enrollment flow with auth modal |
...
### BLS Salary Data (Session 57-58)

**Migration Created:** `004_pathway_career_data.sql`

Adds to courses table:
- `sequence_order` - Course order within pathway syllabus
- `is_required` - Required vs elective
- `prerequisite_course_id` - Course dependencies

**Related Tables Created:**
- `content_sources` - External content providers (freeCodeCamp, Google, HubSpot)
