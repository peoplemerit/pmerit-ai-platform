#!/bin/bash
# =================================================================
# Deploy PMERIT Database Schema with Correct Credentials
# =================================================================

echo "🎯 DEPLOYING PMERIT DATABASE SCHEMA"
echo "===================================="

# Method 1: Use pmerit_admin credentials (most suitable for PMERIT platform)
echo "🚀 OPTION 1: Deploy to pmerit-ai-db-alt container"
echo "Database: pmerit_ai | User: pmerit_admin | Container: pmerit-ai-db-alt"
echo ""

# Create updated schema for pmerit_admin user
cat > database_schema_pmerit.sql << 'SCHEMA_EOF'
-- =================================================================
...
echo "✅ Database schema file created: database_schema_pmerit.sql"
echo ""

# Deploy to pmerit-ai-db-alt (the working container)
echo "🚀 Deploying to pmerit-ai-db-alt container..."
export PGPASSWORD="pmerit_secure_2024"
psql -h localhost -p 15432 -U pmerit_admin -d pmerit_ai -f database_schema_pmerit.sql

if [ $? -eq 0 ]; then
    echo ""
    echo "🎉 SUCCESS! Database schema deployed successfully!"
    echo ""
    echo "✅ Verification - Let's check what was created:"
    psql -h localhost -p 15432 -U pmerit_admin -d pmerit_ai -c "\dt"
    
    echo ""
    echo "✅ Check users table:"
    psql -h localhost -p 15432 -U pmerit_admin -d pmerit_ai -c "SELECT COUNT(*) as user_count FROM users;"
    
    echo ""
    echo "🎯 NEXT STEPS:"
    echo "1. ✅ Database schema deployed successfully"
    echo "2. 🔧 Update super admin password hash"
    echo "3. 🚀 Deploy the backend API (Phase 1 complete)"
    echo "4. 📧 Move to Phase 2: Email integration"
    ...
    echo ""
    echo "🚀 OPTION 2: Deploy to gabriel-db container"
    echo "Database: gabriel_ai | User: gabriel_user"
    
    # Try with gabriel_user credentials
    export PGPASSWORD="gabriel_secure_2025"
    
    # Update schema for gabriel database
    sed 's/\\c pmerit_ai;/\\c gabriel_ai;/' database_schema_pmerit.sql > database_schema_gabriel.sql
    
    psql -h localhost -p 15432 -U gabriel_user -d gabriel_ai -f database_schema_gabriel.sql
    
    if [ $? -eq 0 ]; then
        echo "🎉 SUCCESS with gabriel_user credentials!"
    else
        echo "❌ Both attempts failed. Let's use direct container method..."
        
        # Copy schema to container and run directly
        docker cp database_schema_pmerit.sql pmerit-ai-db-alt:/tmp/
        docker exec pmerit-ai-db-alt psql -U pmerit_admin -d pmerit_ai -f /tmp/database_schema_pmerit.sql
        
        echo "✅ Direct container deployment attempted"
    fi
fi

echo ""
echo "================================================================"
echo "🌍 PMERIT AI MULTI-TIER PORTAL - PHASE 1 STATUS"
echo "================================================================"
echo "✅ Database credentials discovered"
echo "✅ Schema deployment attempted"
echo "🎯 Ready for API deployment and Phase 2 (Email integration)"
